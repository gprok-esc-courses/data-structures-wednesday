...
class Graph:
    ...
    def add_edge(self, ida, idb, weight=1):
        if ida in self.vertices and idb in self.vertices:
            edge = Edge(self.vertices[idb], weight)
            self.vertices[ida].add_edge(edge)
            if not self.directed:
                edge = Edge(self.vertices[ida], weight)
                self.vertices[idb].add_edge(edge)
    ...
    def display(self):
        for v in self.vertices:
            ...
            for edge in self.vertices[v].edges:
                print('[', edge.destination.id, edge.weight, '] ', end='')
            print()
    ...
    def bellman_ford(self, start):
        ...
        for i in range(len(self.vertices)):
            for key in self.vertices:
                ...
                for edge in vertex.edges:
                    self.relax(vertex, edge.destination, edge.weight)
        for key in self.vertices:
            vertex = self.vertices[key]
            for edge in vertex.edges:
...
if __name__ == "__main__":
    ...
    vertices = ['s', 't', 'x', 'y', 'z']
    edges = [
        ['s', 't', 6], ['s', 'y', 7], ['t', 'x', 5], ['x', 't',-8], ['t', 'y', 8], ['y', 'z', 9],
    ...
    result = g.bellman_ford('s')
    if result:
        g.display()
        g.print_shortest_path('s', 't')
        print(g.path)
    else:
